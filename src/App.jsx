...
const dbPatch = (t,id,b) => fetch(SUPA_URL+"/rest/v1/"+t+"?id=eq."+id,{method:"PATCH",headers:{...hdr(),"Prefer":"return=representation"},body:JSON.stringify(b)});
...
const dbDel = (t,id) => fetch(SUPA_URL+"/rest/v1/"+t+"?id=eq."+id,{method:"DELETE",headers:hdr()});
...
const mkItems = (ov) => {
  const o=ov||{}, items={};
  CL_SECTIONS.forEach(s=>s.items.forEach(n=>{const k=s.id+"_"+n;items[k]={state:"",obs:"",urgency:"",images:[],reqId:null,...(o[k]||{})}}));
  return items;
};
...
const normalizeInsp = i => ({items:mkItems(),...(i||{}),items:mkItems((i||{}).items)});
...
function LoginScreen({onLogin}){
  ...
  const [newPass,setNewPass]=useState("");const [newPassC,setNewPassC]=useState("");
  const [passErr,setPassErr]=useState("");const [passOk,setPassOk]=useState(false);const [passLoad,setPassLoad]=useState(false);
  ...
  const submitCambioPass=async()=>{
    ...
    if(newPass!==newPassC){setPassErr("Las contraseñas no coinciden");return;}
    setPassLoad(true);setPassErr("");
    try{
      const res=await fetch(SUPA_URL+"/auth/v1/user",{method:"PUT",headers:{...hdr(recoveryToken),"Content-Type":"application/json"},body:JSON.stringify({password:newPass})});
      if(!res.ok) throw new Error("Error al cambiar contraseña");
      setPassOk(true);window.location.hash="";
  ...
  if(recoveryToken) return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#f1f5f9",fontFamily:"system-ui,sans-serif",padding:16}}>
      <div style={{background:"#fff",borderRadius:16,padding:"40px 32px",width:"100%",maxWidth:400,boxShadow:"0 4px 24px rgba(0,0,0,.08)"}}>
        <div style={{textAlign:"center",marginBottom:28}}><div style={{fontSize:48,marginBottom:12}}>🔐</div><div style={{fontWeight:700,fontSize:20}}>Crear nueva contraseña</div></div>
        {passOk&&<div style={alrt("success")}>Contraseña actualizada. Redirigiendo...</div>}
        {passErr&&<div style={alrt("error")}>{passErr}</div>}
        {!passOk&&<><div style={{marginBottom:14}}><label style={lbl}>Nueva contraseña</label><input style={inp} type="password" value={newPass} onChange={e=>setNewPass(e.target.value)} placeholder="Mínimo 6 caracteres"/></div><div style={{marginBottom:20}}><label style={lbl}>Confirmar contraseña</label><input style={inp} type="password" value={newPassC} onChange={e=>setNewPassC(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitCambioPass()}/></div><button style={{width:"100%",padding:"12px",background:"#3b82f6",color:"#fff",border:"none",borderRadius:8,fontSize:15,fontWeight:600,cursor:"pointer"}} onClick={submitCambioPass} disabled={passLoad}>{passLoad?"Guardando...":"Guardar contraseña"}</button></>}
      </div>
    </div>
  );
  ...
  if(!mode) return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#f1f5f9",fontFamily:"system-ui,sans-serif",padding:16}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:40}}><div style={{background:"#0f172a",borderRadius:16,width:64,height:64,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",fontSize:28}}>🏢</div><div style={{fontWeight:700,fontSize:24,color:"#0f172a"}}>CondoAdmin</div><div style={{color:"#64748b",fontSize:14,marginTop:4}}>Sistema de Gestión de Condominios</div></div>
  ...
  return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0f172a",fontFamily:"system-ui,sans-serif",padding:16}}>
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <div style={{textAlign:"center",marginBottom:32}}><div style={{fontSize:48,marginBottom:12}}>🔑</div><div style={{fontWeight:700,fontSize:22,color:"#fff"}}>Acceso Personal</div><div style={{color:"#64748b",fontSize:13,marginTop:4}}>Administración · Conserjería · Comité</div></div>
        ...
        <div style={{marginBottom:14}}><label style={{...lbl,color:"#94a3b8"}}>Correo electrónico</label><input style={{...inp,background:"#1e293b",border:"1px solid #334155",color:"#fff"}} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="correo@ejemplo.cl" onKeyDown={e=>e.key==="Enter"&&submitPersonal()}/></div>
        <div style={{marginBottom:8}}><label style={{...lbl,color:"#94a3b8"}}>Contraseña</label><input style={{...inp,background:"#1e293b",border:"1px solid #334155",color:"#fff"}} type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&submitPersonal()}/></div>
        <div style={{textAlign:"right",marginBottom:20}}>
          <button onClick={async()=>{
            if(!email||!email.includes("@")){setErr("Ingrese su correo primero");return;}
            try{const res=await fetch(SUPA_URL+"/auth/v1/recover",{method:"POST",headers:{"Content-Type":"application/json","apikey":SUPA_KEY},body:JSON.stringify({email})});if(res.ok)setErr("✓ Mail de recuperación enviado a "+email);else setErr("Error al enviar.");}catch(e){setErr("Error al enviar.");}
...
export default function App(){
  ...
  const persist = async (table,item,idField="id") => {
    try { await dbUpsert(table,{[idField]:item[idField],data:item}); }
    catch(e){ console.error("persist",table,e); }
  };
  const persistCfg = async (key,data) => { try { await dbUpsert("config",{key,data}); } catch(e){ console.error("cfg",e); }};
  ...
  const setDispatchDB=updater=>setDispatch(prev=>{const next=typeof updater==="function"?updater(prev):updater;next.forEach(item=>{if(!prev.find(x=>x.id===item.id))persist("despachos",item);});return next;});
        ...
        {(!mob||navOpen)&&(
          <div onClick={e=>e.stopPropagation()} style={{width:mob?260:220,background:"#0f172a",display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto",position:mob?"fixed":"relative",top:0,left:0,bottom:0,height:mob?"100vh":"auto",zIndex:50}}>
            ...
            <nav style={{padding:"6px 0",flex:1}}>
              {navItems.map(n=>(
                <div key={n.id} onClick={()=>go(n.id)} style={{padding:"10px 14px",cursor:"pointer",userSelect:"none",fontSize:13,color:isAct(n.id)?"#fff":"#94a3b8",background:isAct(n.id)?"#1e3a5f":"transparent",borderLeft:isAct(n.id)?"3px solid #3b82f6":"3px solid transparent",fontWeight:isAct(n.id)?600:400}}>{n.label}</div>
              ))}
            </nav>
          ...
          <tr key={r.id} style={{background:r.priority==="Emergencia"?"#fef2f2":"",cursor:"pointer"}} onClick={()=>onOpen(r)}>
            ...
            <td style={tdSt} onClick={e=>e.stopPropagation()}>
              <div style={{display:"flex",gap:4}}>
                ...
                {can(role,"manageConfig")&&<button style={mkBtn("danger",true)} onClick={e=>{e.stopPropagation();if(window.confirm("¿Eliminar "+r.code+"?"))setReqs(p=>p.filter(x=>x.id!==r.id));}}>🗑</button>}
              </div>
            </td>
          </tr>
        ))}</tbody></table></div>
...
function ReqDetail({req,reqs,tasks,atts,emails,role,setReqs,setTasks,setAtts,addEmail,showToast,onBack,setSelReq,mob,respList,respAssign,usuarios}){
  ...
  const applyAsgn=async()=>{
    ...
    try{
      ...
      if(u?.email) addEmail({requestId:r.id,date:new Date().toISOString(),to:u.email,subject:"[CondoAdmin] Solicitud "+r.code+" asignada",type:"Asignacion",status:"Enviado",body:"Hola "+u.nombre+",\n\nSe te asignó la solicitud "+r.code+".\n\nCategoría: "+r.category+"\nDescripción: "+r.description+"\nPrioridad: "+r.priority});
    }catch(e){console.error(e);}
  };
  const addCmt=()=>{if(!comment.trim()) return;upd({comments:[...safeComments,{id:"c"+uid(),user:role,role,date:new Date().toISOString(),text:comment}]});setComment("");showToast("Comentario agregado");};
  const closeFinal=()=>{
  ...
  const tabs=[{id:"info",label:"Info"},{id:"history",label:"Historial ("+safeHistory.length+")"},{id:"tasks",label:"Órdenes ("+myTasks.length+")"},{id:"evidence",label:"Evidencias"},{id:"emails",label:"Correos ("+myEmails.length+")"}];
  return(
    <div>
      ...
      {(can(role,"changeStatus")||can(role,"assign"))&&r.status!=="Cerrada"&&r.status!=="Rechazada"&&(
        <div style={{...card,padding:12,marginBottom:12}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
            {can(role,"changeStatus")&&<div><label style={lbl}>Estado</label><div style={{display:"flex",gap:6}}><select style={{...sel,width:140}} value={ns} onChange={e=>setNs(e.target.value)}>{STATUSES.map(s=><option key={s}>{s}</option>)}</select><button style={mkBtn("primary",true)} onClick={applyStatus}>OK</button></div></div>}
            {can(role,"assign")&&<div><label style={lbl}>Responsable</label><div style={{display:"flex",gap:6}}><select style={{...sel,width:150}} value={asgn} onChange={e=>setAsgn(e.target.value)}>{respList.map(s=><option key={s}>{s}</option>)}</select><button style={mkBtn("secondary",true)} onClick={applyAsgn}>Asignar</button></div></div>}
            {can(role,"createTask")&&<button style={mkBtn("secondary",true)} onClick={()=>setShowTF(true)}>+ Orden</button>}
  ...
  return(
    <div style={{...card,border:"2px solid #3b82f6",marginBottom:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <div style={fg}><label style={lbl}>Responsable (supervisa)</label><select style={sel} value={f.responsible} onChange={e=>setF(p=>({...p,responsible:e.target.value}))}>{respAssign.map(r=><option key={r}>{r}</option>)}</select></div>
    ...
    <div style={{...card,marginBottom:10,borderLeft:"4px solid "+(PC[task.priority]||"#e2e8f0")}}>
      ...
      {exp&&<div style={{marginTop:10,borderTop:"1px solid #f1f5f9",paddingTop:10}}>
        ...
        {task.informe&&<div style={{...alrt(task.estadoFinal==="Resuelto"?"success":task.estadoFinal==="Resuelto parcialmente"?"warning":"error"),marginBottom:10}}>
          ...
          {task.herramientas&&<div style={{fontSize:11,marginTop:4}}>🔧 {task.herramientas}</div>}
        ...
        <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
          ...
          <button style={mkBtn("secondary",true)} onClick={()=>setShowEv(true)}>📷</button>
  ...
  return(
    <div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:580,padding:"20px",marginTop:16,marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <button style={{...mkBtn(esc?"danger":"purple"),flex:1,justifyContent:"center"}} onClick={esc?detenerVoz:iniciarVoz}>{esc?"⏹ Detener":"🎤 Dictar por voz"}</button>
      ...
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:14}}>
        ...
        <button style={mkBtn("success",true)} onClick={()=>{if(!f.texto.trim()){alert("Ingrese la descripción");return;}if(!f.vistoBueno){alert("Debe confirmar el trabajo");return;}onSave(f);}}>Guardar informe</button>
      </div>
    </div></div>
  );
}
...
function EvidModal({type,requestId,role,atts,setAtts,showToast,onClose,taskId,setTasks}){
  ...
  const handleFiles=e=>{
    ...
    files.forEach((f,i)=>{
      ...
      r.onload=ev=>setPrev(p=>[...p,{name:f.name,url:ev.target.result}]);
      r.readAsDataURL(f);
    });
  ...
  const save=async()=>{
    ...
    try{
      const uploaded=await Promise.all(previews.map(async(p,i)=>{
        ...
        let url=p.url;
        try{ if(rawFiles[i]) url=await uploadImg(rawFiles[i],path); }catch(e){ console.error("storage",e); }
  ...
  return(
    <div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:480,padding:"20px",marginTop:16}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><h3 style={{margin:0,fontSize:14}}>📷 Evidencia - {type}</h3><button style={mkBtn("ghost",true)} onClick={onClose}>✕</button></div>
...
function CloseModal({req,atts,setAtts,role,onClose,onConfirm,showToast}){
  ...
  return(
    <div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:440,padding:"20px",marginTop:16}}>
      ...
      {closure.length===0?<div><div style={alrt("error")}>Cargue al menos una imagen de cierre.</div><button style={mkBtn("primary",true)} onClick={()=>setShowEv(true)}>Cargar evidencia</button></div>:<div><div style={alrt("success")}>{closure.length} imagen(es) de cierre.</div><p style={{fontSize:13}}>¿Confirmar cierre definitivo?</p></div>}
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:14}}><button style={mkBtn("secondary",true)} onClick={onClose}>Cancelar</button>{closure.length>0&&<button style={mkBtn("success",true)} onClick={onConfirm}>Confirmar</button>}</div>
...
function NewReqModal({role,reqs,setReqs,addEmail,showToast,onClose,onOpen,cats,towers,session,usuarios}){
  ...
  const handleFiles=e=>Array.from(e.target.files).forEach((fi,i)=>{const r=new FileReader();r.onload=ev=>setPrevs(p=>[...p,{name:fi.name,url:ev.target.result}]);r.readAsDataURL(fi);setRawFiles(p=>[...p,fi]);});
  ...
  const submit=async()=>{
    ...
    setReqs(p=>[nr,...p]);
    addEmail({requestId:code,date:now,to:f.requesterEmail,subject:"[CondoAdmin] Solicitud "+code+" recibida",type:"Creacion",status:"Enviado",body:"Su solicitud fue registrada. Código: "+code+". Nos contactaremos a la brevedad."});
    try{
      const admins=(usuarios||[]).filter(u=>["Administrador","Administrador Edificio"].includes(u.rol)&&u.email);
      admins.forEach(u=>addEmail({requestId:code,date:now,to:u.email,subject:"[CondoAdmin] Nueva solicitud "+code+" - "+(tipo==="Administrativo"?adminCat:f.category),type:"Aviso",status:"Enviado",body:"Nueva solicitud:\n\nCódigo: "+code+"\nTipo: "+tipo+"\nSolicitante: "+f.requesterName+"\nCorreo: "+f.requesterEmail+"\nTorre: "+f.tower+" / Unidad: "+f.unit+"\nCategoría: "+(tipo==="Administrativo"?adminCat+" - "+adminSub:f.category+" - "+f.subcategory)+"\nPrioridad: "+f.priority+"\n\nDescripción:\n"+f.description}));
    }catch(e){}
  ...
  return(
    <div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:680,padding:"20px",marginTop:16,marginBottom:16}}>
      ...
      {isAdmin&&(
        <div style={{display:"flex",gap:10,marginBottom:16}}>
          {["Administrativo","Incidencia"].map(t=>(
            <button key={t} onClick={()=>setTipo(t)} style={{flex:1,padding:"12px",borderRadius:10,border:"2px solid "+(tipo===t?(t==="Administrativo"?"#6366f1":"#ef4444"):"#e2e8f0"),background:tipo===t?(t==="Administrativo"?"#eef2ff":"#fef2f2"):"#f9fafb",color:tipo===t?(t==="Administrativo"?"#6366f1":"#ef4444"):"#6b7280",fontWeight:tipo===t?700:400,cursor:"pointer",fontSize:14}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <div style={{...fg,gridColumn:"1/-1"}}><label style={lbl}>{tipo==="Administrativo"?"Descripción / Nota *":"Descripción *"}</label><textarea style={{...inp,height:tipo==="Administrativo"?120:80,resize:"vertical",borderColor:errs.description?"#ef4444":""}} value={f.description} onChange={e=>set("description",e.target.value)} placeholder={tipo==="Administrativo"?"Detalle administrativo...":"Describa el problema..."}/>{errs.description&&<div style={{color:"#ef4444",fontSize:10}}>{errs.description}</div>}</div>
      ...
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:8}}><button style={mkBtn("secondary",true)} onClick={onClose}>Cancelar</button><button style={mkBtn(tipo==="Administrativo"?"purple":"primary",true)} onClick={submit} disabled={saving}>{saving?"Guardando...":"Enviar"}</button></div>
    </div></div>
  );
}
...
function TasksView({tasks,reqs,role,setTasks,showToast,mob,respAssign}){
  ...
  return(
    <div>
      ...
      {visible.length===0?<Empty msg="Sin órdenes de trabajo"/>:visible.map(t=>{const req=reqs.find(r=>r.id===t.requestId);return(<div key={t.id} style={{...card,borderLeft:"4px solid "+(PC[t.priority]||"#e2e8f0"),marginBottom:8,padding:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}><div style={{minWidth:0}}><div style={{fontWeight:600,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.title}</div>{req&&<div style={{fontSize:10,color:"#3b82f6"}}>{req.code} - {req.category}</div>}<div style={{fontSize:11,color:"#64748b"}}>👤 {t.responsible}{t.ejecutor&&" · 🔧 "+t.ejecutor}</div></div><div style={{display:"flex",gap:6}}><PBadge p={t.priority}/><SBadge s={t.status}/></div></div></div>);})}
    </div>
  );
}
...
function ProviderDash({role,showToast,mob,reqs,session}){
  ...
  return(
    <div>
      ...
      {myReqs.length===0?<Empty msg="No tienes solicitudes asignadas"/>:<div>{myReqs.map(r=>(
        <div key={r.id} style={{...card,borderLeft:"4px solid "+(PC[r.priority]||"#e2e8f0"),marginBottom:10,padding:14}}>
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4,flexWrap:"wrap"}}><span style={{fontWeight:700,color:"#3b82f6"}}>{r.code}</span><PBadge p={r.priority}/><SBadge s={r.status}/></div>
          <div style={{fontWeight:600,fontSize:14,marginBottom:3}}>{r.category} - {r.subcategory}</div>
          <div style={{fontSize:11,color:"#64748b"}}>Torre {r.tower} / Unidad {r.unit}</div>
...
function Inspections({inspections,setInsp,reqs,setReqs,addEmail,showToast,role,mob,towers}){
  ...
  return(
    <div>
      ...
      <div style={{...card,padding:12,marginBottom:12,display:"flex",justifyContent:"flex-end"}}>{!readOnly&&<button style={mkBtn("primary",true)} onClick={()=>setSub("new")}>+ Nueva inspección</button>}</div>
      {inspections.length===0?<Empty msg="Sin inspecciones"/>:<div>{[...inspections].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(ins=>{
          ...
          {sec.items.map(name=>{
            ...
            return(
              <div key={name} style={{borderBottom:"1px solid #f1f5f9",paddingBottom:12,marginBottom:12}}>
                ...
                {it.state&&it.state!=="No aplica"&&<div style={{display:"flex",gap:8,marginBottom:6}}><input style={{...inp,flex:1,fontSize:12}} placeholder="Observación..." value={it.obs} onChange={e=>setItem(sec.id,name,"obs",e.target.value)}/>{isMalo&&<select style={{...sel,width:110,fontSize:12}} value={it.urgency} onChange={e=>setItem(sec.id,name,"urgency",e.target.value)}><option value="">Urgencia...</option>{URGENCY_LEVELS.map(u=><option key={u}>{u}</option>)}</select>}</div>}
                {(it.state==="Malo"||it.state==="Regular")&&<div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}><button style={mkBtn("secondary",true)} onClick={()=>{setPendImg({sid:sec.id,name});fileRef.current.click();}}>Foto</button>{(it.images||[]).map((img,i)=><img key={i} src={img} alt="" style={{...thumb,width:50,height:40}}/>)}{isMalo&&!it.reqId&&<button style={mkBtn("danger",true)} onClick={()=>createReq(sec.id,name)}>+ Solicitud</button>}{isMalo&&it.reqId&&<span style={bdg("#10b981","#f0fdf4")}>✓ {it.reqId}</span>}</div>}
              </div>
            );
          })}
...
function InvView({inventory,setInv,dispatch,setDispatch,reqs,role,showToast,mob,respAssign}){
  ...
  const [showForm,setShowForm]=useState(false);const [editItem,setEditItem]=useState(null);const [movement,setMov]=useState(null);
  ...
  return(
    <div>
      ...
      <div style={{...card,padding:12,marginBottom:12}}><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><input style={{...inp,flex:2,minWidth:120}} placeholder="Buscar..." value={fi.q} onChange={e=>setFi(p=>({...p,q:e.target.value}))}/><select style={{...sel,flex:1}} value={fi.cat} onChange={e=>setFi(p=>({...p,cat:e.target.value}))}><option value="">Todas las categorías</option>{INV_CATS.map(c=><option key={c}>{c}</option>)}</select>{!readOnly&&<button style={mkBtn("primary",true)} onClick={()=>{setEditItem(null);setShowForm(true);}}>+ Agregar</button>}</div></div>
      {visible.length===0?<Empty msg="Sin insumos"/>:<div>{visible.map(item=>{const low=item.stock<item.minStock;return(<div key={item.id} style={{...card,borderLeft:"4px solid "+(low?"#ef4444":"#10b981"),padding:12,marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>{item.name}</div><div style={{fontSize:11,color:"#64748b"}}>{item.category} - {item.location}</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:700,fontSize:16,color:low?"#ef4444":"#1e293b"}}>{item.stock} <span style={{fontSize:11,fontWeight:400}}>{item.unit}</span></div><div style={{fontSize:10,color:"#94a3b8"}}>min: {item.minStock}</div></div></div>{!readOnly&&<div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}><button style={mkBtn("secondary",true)} onClick={()=>setMov({item,dir:1})}>+ Stock</button><button style={mkBtn("secondary",true)} onClick={()=>setMov({item,dir:-1})}>- Stock</button><button style={mkBtn("ghost",true)} onClick={()=>{setEditItem(item);setShowForm(true);}}>Editar</button><button style={mkBtn("ghost",true)} onClick={()=>setInv(p=>p.filter(i=>i.id!==item.id))}>Eliminar</button></div>}</div>);})}</div>}
...
function EmailsView({logs,setEmails,role}){
  ...
  return(<div>
    <div style={{...card,padding:12,marginBottom:12,display:"flex",gap:8}}>
      ...
      {can(role,"manageConfig")&&logs.length>0&&<button style={mkBtn("danger",true)} onClick={()=>{if(window.confirm("¿Eliminar todos los correos?"))setEmails([]);}}>🗑 Limpiar</button>}
    </div>
    <div style={card}>
...
function MantView({mant,setMant,reqs,role,showToast,addEmail,mob,respList,towers,equipos,setEquipos,certs,setCerts}){
  ...
  if(sub==="detail"&&sel){
    ...
    return <MantDetail item={item} mant={mant} setMant={setMant} role={role} showToast={showToast} mob={mob} readOnly={readOnly} onBack={()=>setSub("list")} respList={respList}/>;
  }
  ...
  return(
    <div>
      ...
      {mantTab==="mant"&&<div>
        ...
        {visible.length===0?<Empty msg="Sin mantenciones"/>:<div>{visible.map(m=>{
      ...
      {mantTab==="certs"&&<div>
        {activeEquipos.length===0?<div style={alrt("warning")}>No hay equipos activos. Ve a Config → Equipos.</div>:<div>
          <div style={{...card,padding:12,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:13,color:"#64748b"}}>{activeEquipos.length} equipos activos</div>{!readOnly&&<button style={mkBtn("primary",true)} onClick={()=>{setEditCert(null);setShowCertForm(true);}}>+ Nueva certificación</button>}</div>
            ...
            return(
              <div key={eq.id} style={{...card,marginBottom:10,borderLeft:"4px solid "+cColor}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:22}}>{eq.icono}</span><div><div style={{fontWeight:700,fontSize:14}}>{eq.nombre}</div><div style={{fontSize:11,color:"#64748b"}}>Frecuencia: {eq.frecuencia}</div></div></div>
                ...
                {equipCerts.length>1&&<details style={{marginTop:8}}><summary style={{fontSize:11,color:"#6366f1",cursor:"pointer"}}>Historial ({equipCerts.length})</summary><div>{equipCerts.map(c=><div key={c.id} style={{fontSize:11,padding:"4px 0",borderBottom:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between"}}><span>{fmtD(c.fecha)} &#8594; {fmtD(c.vencimiento)}</span><span style={{color:"#64748b"}}>{c.empresa}</span></div>)}</div></details>}
              </div>
            );
          })}
...
function CertForm({cert,equipos,towers,onSave,onClose}){
  ...
  return(<div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:520,padding:"20px",marginTop:16,marginBottom:16}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <div style={fg}><label style={lbl}>Ubicación específica</label><input style={inp} placeholder="ej: Subterráneo..." value={f.ubicacion} onChange={e=>set("ubicacion",e.target.value)}/></div>
...
function MantForm({item,onSave,onClose,respList}){
  ...
  const [f,setF]=useState({asset:"",category:defCat,subcategory:MANT_SUBCATS[defCat][0],location:"",tipo:MANT_TIPOS[0],responsible:respList[0]||"",provider:"",lastDate:"",nextDate:"",costEstimated:"",costReal:"",description:"",status:"Vigente",...(item||{})});
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  const subs=MANT_SUBCATS[f.category]||[];
  const handleCat=e=>{const nc=e.target.value;const ns=MANT_SUBCATS[nc]||[];set("category",nc);if(!ns.includes(f.subcategory))set("subcategory",ns[0]||"");};
  return(<div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:600,padding:"20px",marginTop:16,marginBottom:16}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}><h3 style={{margin:0,fontSize:15}}>{item?"Editar":"Nueva"} mantención</h3><button style={mkBtn("ghost",true)} onClick={onClose}>✕</button></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <div style={fg}><label style={lbl}>Categoría</label><select style={sel} value={f.category} onChange={handleCat}>{MANT_CATS.map(c=><option key={c}>{c}</option>)}</select></div>
...
function MantDetail({item,mant,setMant,role,showToast,mob,readOnly,onBack,respList}){
  ...
  return(
    <div>
      ...
      {!readOnly&&<div style={{...card,padding:12,marginBottom:12}}><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><select style={{...sel,width:160}} value={m.status} onChange={e=>upd({status:e.target.value})}>{MANT_ESTADOS.map(s=><option key={s}>{s}</option>)}</select><button style={mkBtn("success",true)} onClick={()=>setShowHF(true)}>+ Registrar ejecución</button><button style={mkBtn("secondary",true)} onClick={()=>setShowDF(true)}>+ Documento</button></div></div>}
      <Tabs tabs={tabs} active={tab} onChange={setTab} accent="#6366f1"/>
...
function HistForm({onSave,onClose,respList}){
  const [f,setF]=useState({date:new Date().toISOString().slice(0,10),tipo:MANT_TIPOS[0],responsible:respList[0]||"",notes:"",costReal:0,marcarVigente:false});
  return(<div style={{...card,border:"2px solid #6366f1",marginBottom:12}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><div style={{fontWeight:600,fontSize:13}}>Registrar ejecución</div><button style={mkBtn("ghost",true)} onClick={onClose}>✕</button></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
...
function ConfigView({cats,setCats,towers,setTowers,equipos,setEquipos,showToast,session,setUsuarios}){
  ...
  const loadUsuarios=async()=>{
    try{const res=await fetch(SUPA_URL+"/rest/v1/usuarios?order=created_at.asc&select=*",{headers:hdr(session.token)});const data=await res.json();const list=Array.isArray(data)?data:[];setUsuariosLocal(list);setUsuarios(list);}catch(e){console.error(e);}
  };
  ...
  return(
    <div>
      ...
      {tab==="towers"&&<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:13,color:"#64748b"}}>{towers.filter(t=>t.active).length} activas</div><button style={mkBtn("primary",true)} onClick={()=>{setEditTow(null);setShowTF(true);}}>+ Nueva torre</button></div><div style={card}>{towers.map(t=><div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 0",borderBottom:"1px solid #f1f5f9",opacity:t.active?1:.5,flexWrap:"wrap"}}><div style={{width:34,height:34,borderRadius:8,background:"#eff6ff",border:"1px solid #bfdbfe",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:14,color:"#3b82f6"}}>{t.name}</div><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>{t.label}</div></div><div style={{display:"flex",gap:4}}><button style={mkBtn("secondary",true)} onClick={()=>{setEditTow(t);setShowTF(true);}}>Editar</button><button style={mkBtn(t.active?"warning":"success",true)} onClick={()=>toggleTow(t.id)}>{t.active?"Desact.":"Activar"}</button><button style={mkBtn("danger",true)} onClick={()=>setTowers(p=>p.filter(x=>x.id!==t.id))}>X</button></div></div>)}</div>{showTF&&<TowerForm tower={editTow} onSave={saveTow} onClose={()=>{setShowTF(false);setEditTow(null);}}/>}</div>}
      {tab==="equipos"&&<div>
        <div style={{...alrt("info"),marginBottom:12}}>Activa los equipos que existen en tu edificio.</div>
        <div style={card}>{EQUIP_TIPOS.map(et=>{const eq=(equipos||[]).find(e=>e.id===et.id)||{...et,active:false};return(<div key={et.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f1f5f9",opacity:eq.active?1:.5}}><div style={{fontSize:24}}>{et.icono}</div><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>{et.nombre}</div><div style={{fontSize:11,color:"#64748b"}}>{et.frecuencia}</div></div><button style={mkBtn(eq.active?"warning":"success",true)} onClick={()=>setEquipos(p=>p.map(e=>e.id===et.id?{...e,active:!e.active}:e))}>{eq.active?"Desactivar":"Activar"}</button></div>);})}
        </div>
        <div style={{fontSize:12,color:"#64748b",marginTop:8}}>{(equipos||[]).filter(e=>e.active).length} equipos activos</div>
      </div>}
      {tab==="usuarios"&&<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:13,color:"#64748b"}}>{usuariosLocal.filter(u=>u.active).length} activos</div><button style={mkBtn("primary",true)} onClick={()=>{setEditUser(null);setShowUF(true);}}>+ Nuevo usuario</button></div>
        {showUF&&<UserForm user={editUser} onSave={saveUsuario} onClose={()=>{setShowUF(false);setEditUser(null);}}/>}
        <div>{usuariosLocal.map(u=><div key={u.id} style={{...card,opacity:u.active?1:.6,marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>{u.nombre}</div><div style={{fontSize:11,color:"#64748b"}}>{u.email}</div><span style={bdg("#6366f1","#eef2ff")}>{u.rol}</span></div><div style={{display:"flex",gap:4,flexWrap:"wrap"}}><button style={mkBtn("secondary",true)} onClick={()=>{setEditUser(u);setShowUF(true);}}>Editar</button><button style={mkBtn(u.active?"warning":"success",true)} onClick={()=>toggleUser(u)}>{u.active?"Desact.":"Activar"}</button><button style={mkBtn("danger",true)} onClick={()=>deleteUser(u)}>Eliminar</button></div></div></div>)}</div>
      </div>}
...
function UserForm({user,onSave,onClose}){
  ...
  return(<div style={modal}><div style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:480,padding:"20px",marginTop:16}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <div style={{...fg,gridColumn:"1/-1"}}><label style={lbl}>Rol</label><select style={sel} value={rol} onChange={e=>{setRol(e.target.value);setF(p=>({...p,rol:e.target.value}));}}>{ROLES.map(r=><option key={r} value={r}>{r}</option>)}</select></div>
    </div>
